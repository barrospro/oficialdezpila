...
const veo3Reels = [
  {
    mp4Name: "veo3_01_futebol_sem_travar.mp4",
    pngCoverName: "veo3_01_futebol_sem_travar.png",
    title: "⚽ O Gol Decisivo Sem Travamentos",
    accentColor: "#10b981",
    bgGradient: "radial-gradient(circle at center, rgba(16, 185, 129, 0.35) 0%, #050507 75%)",
    icon: "⚽",
  ...
  {
    mp4Name: "veo3_02_cinema_luxo.mp4",
    pngCoverName: "veo3_02_cinema_luxo.png",
    title: "🍿 Cinema de Luxo em Casa 4K",
    accentColor: "#ff2a2a",
    bgGradient: "radial-gradient(circle at center, rgba(255, 42, 42, 0.35) 0%, #050507 75%)",
    icon: "🍿",
  ...
  {
    mp4Name: "veo3_03_fatura_vs_dezpila.mp4",
    pngCoverName: "veo3_03_fatura_vs_dezpila.png",
    title: "⚡ O Antes vs Depois da Fatura",
    accentColor: "#f59e0b",
    bgGradient: "radial-gradient(circle at center, rgba(245, 158, 11, 0.35) 0%, #050507 75%)",
    icon: "⚡",
  ...
  {
    mp4Name: "veo3_04_ativacao_2min.mp4",
    pngCoverName: "veo3_04_ativacao_2min.png",
    title: "📲 Ativação Rápida no Celular & TV",
    accentColor: "#06b6d4",
    bgGradient: "radial-gradient(circle at center, rgba(6, 182, 212, 0.35) 0%, #050507 75%)",
    icon: "📲",
  ...
  {
    mp4Name: "veo3_05_cristoflix_familia.mp4",
    pngCoverName: "veo3_05_cristoflix_familia.png",
    title: "🧸 Família Reunida & CristoFlix Infantil",
    accentColor: "#a855f7",
    bgGradient: "radial-gradient(circle at center, rgba(168, 85, 247, 0.35) 0%, #050507 75%)",
    icon: "✝️",
  ...
  {
    mp4Name: "veo3_06_manifesto_epic.mp4",
    pngCoverName: "veo3_06_manifesto_epic.png",
    title: "🔥 Trailer Épico Dark / Manifesto DezPila",
    accentColor: "#ef4444",
    bgGradient: "radial-gradient(circle at center, rgba(239, 68, 68, 0.4) 0%, #050507 75%)",
    icon: "🔥",
...
console.log("Gerando 6 vídeos virais reais em formato .MP4 (720x1280 px H.264)...");

const FPS = 3;
const NUM_FRAMES = 3; // 3 frames a 3 FPS = 1.0s video loop

veo3Reels.forEach((item, idx) => {
  const framesDir = path.join(__dirname, `temp_frames_${idx}`);
  if (!fs.existsSync(framesDir)) {
    fs.mkdirSync(framesDir, { recursive: true });
  }

  const outMp4Path = path.join(outDir, item.mp4Name);

  console.log(`Renderizando frames para: ${item.mp4Name}...`);

  for (let frame = 0; frame < NUM_FRAMES; frame++) {
    const progress = frame / NUM_FRAMES; // 0.0 -> 1.0
    const pulseScale = 1 + Math.sin(progress * Math.PI * 4) * 0.05;
    const glowBlur = 30 + Math.sin(progress * Math.PI * 4) * 15;
    const ctaScale = 1 + Math.cos(progress * Math.PI * 4) * 0.03;
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .bg-glow {
    ...
    background:${item.bgGradient}; top:50%; left:50%; transform:translate(-50%,-50%) scale(${pulseScale});
    pointer-events:none;
  }
  ...
  .main-card {
    ...
    box-shadow:0 25px 60px rgba(0,0,0,0.9), 0 0 ${glowBlur}px ${item.accentColor}55;
  ...
  .icon-badge {
    ...
    box-shadow:0 0 ${glowBlur}px ${item.accentColor}66; margin-bottom:5px;
  }
  .category-badge {
  ...
  .cta-box {
    ...
    box-shadow:0 15px 35px rgba(151,2,2,0.7), 0 0 ${glowBlur}px rgba(255,77,77,0.5);
...
<body>
  ...
  <div class="cta-box">
    ...
    const tempHtml = path.join(framesDir, `frame_${frame}.html`);
    fs.writeFileSync(tempHtml, htmlContent, "utf-8");

    try {
      execSync(
        `"${browserPath}" --headless --screenshot="${frameFile}" --window-size=720,1280 --hide-scrollbars "${tempHtml}"`
      );
    } catch {
      // Ignora pequenos avisos
    } finally {
      if (fs.existsSync(tempHtml)) fs.unlinkSync(tempHtml);
    }
  }

  // Compila os frames no vídeo .MP4 usando FFMPEG
  console.log(`Compilando .MP4 com FFMpeg: ${item.mp4Name}...`);
  try {
    const ffmpegCmd = `"${ffmpegPath}" -y -framerate ${FPS} -i "${framesDir}\\frame_%03d.png" -c:v libx264 -pix_fmt yuv420p "${outMp4Path}"`;
    execSync(ffmpegCmd);
    console.log(`✓ VÍDEO COMPILADO COM SUCESSO: ${outMp4Path}`);
  } catch (err) {
    console.error(`Erro ao compilar ${item.mp4Name}:`, err.message);
  } finally {
...
console.log("\nTODOS OS 6 VÍDEOS REAIS EM .MP4 FORAM GERADOS COM SUCESSO!");
