...
import { MessageCircle, CheckCheck, X, Eye, ShieldCheck, Star } from "lucide-react";
...
// 1. LINHA SUPERIOR: Depoimentos em Texto
const DEPOIMENTOS_TEXTO: DepoimentoTexto[] = [
  {
...
// 2. LINHA INFERIOR: Fotos Reais e Prints do WhatsApp
const PROVAS_FOTOS_REAIS: ProvaFotoReal[] = [
  {
    nome: "Lucas Carvalho",
    cidade: "São Paulo, SP",
    ...
    >
      ...
      <div className="mb-12 px-6 text-center max-w-4xl mx-auto">
        ...
        <p className="text-muted-foreground mt-3 font-code text-xs sm:text-sm max-w-2xl mx-auto">
          Veja o que nossos clientes dizem após ativar o acesso. Passe o mouse para pausar e clique em qualquer foto para ampliar:
        </p>
      </div>

      {/* Conteúdo com 2 Linhas: 1 Linha de Depoimentos Texto + 1 Linha de Fotos Reais Embaixo */}
      <div className="relative space-y-6">
        {/* Efeitos de fade nas laterais */}
        <div
        ...
        {/* ========================================================
            LINHA 1 (SUPERIOR): DEPOIMENTOS EM TEXTO
        ======================================================== */}
        <div className="overflow-hidden">
          ...
          >
            {[...DEPOIMENTOS_TEXTO, ...DEPOIMENTOS_TEXTO].map((c, i) => (
              <div
                key={"depo-txt-" + i}
                onClick={() =>
                  c.proofImg &&
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex gap-0.5 text-amber-400">
                      {Array.from({ length: c.stars }).map((_, sIdx) => (
                        <Star key={sIdx} className="w-3.5 h-3.5 fill-amber-400" />
                      ))}
                    </div>
                    <span className="font-code text-[9px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 uppercase tracking-widest rounded flex items-center gap-1">
                      <CheckCheck className="w-2.5 h-2.5" /> Verificado
                    </span>
                  </div>
                  <p className="text-xs leading-relaxed text-slate-200 font-body mb-3">
                    "{c.q}"
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-white/5">
                  <div className="flex items-center gap-2.5">
                    <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-[10px] font-bold font-code bg-brand/20 border border-brand/40 text-brand">
                      {c.iniciais}
                    </div>
                    <div>
                      ...
                      <div className="text-[10px] font-code text-slate-400">
                        {c.cidade}
                      </div>
                    </div>
                  </div>
                  {c.proofImg && (
                    <span className="text-[10px] font-code text-emerald-400 hover:text-emerald-300 underline underline-offset-2 flex items-center gap-1">
                      <Eye className="w-3 h-3" /> Ver Print
                    </span>
                  )}
                </div>
              </div>
        ...
        {/* ========================================================
            LINHA 2 (INFERIOR): FOTOS REAIS / PRINTS DO WHATSAPP
        ======================================================== */}
        <div className="overflow-hidden pt-2">
          ...
          >
            {[...PROVAS_FOTOS_REAIS, ...PROVAS_FOTOS_REAIS].map((prova, idx) => (
              <div
                key={"prova-foto-" + idx}
                onClick={() =>
                  setSelectedProof({
              ...
              >
                ...
                {/* Imagem do Print / Foto Real */}
                <div className="relative rounded-xl overflow-hidden aspect-[4/5] bg-black/90 border border-white/10 mb-2.5 group-hover/proof:border-emerald-500/40">
                  <img
                    src={prova.img}
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-transparent to-transparent opacity-75 group-hover/proof:opacity-30 transition-opacity" />
                  <div className="absolute bottom-2.5 left-2.5 right-2.5 flex items-center justify-center gap-1.5 bg-black/80 backdrop-blur-md py-1.5 px-3 rounded-lg border border-white/15 text-[11px] font-code text-white font-bold group-hover/proof:bg-emerald-600 group-hover/proof:border-emerald-400 transition-colors shadow-md">
                ...
                {/* Rodapé da Foto */}
                <div>
                  <div className="flex items-center justify-between text-[10px] font-code text-slate-400 mb-0.5">
                    <span>{prova.cidade}</span>
                    <span className="text-emerald-400 font-bold truncate max-w-[130px] text-right">
                      {prova.dispositivo}
                    </span>
                  </div>
      ...
      {/* ========================================================
          MODAL LIGHTBOX (AMPLIAÇÃO EM ALTA RESOLUÇÃO)
      ======================================================== */}
      {selectedProof && (
        <div
